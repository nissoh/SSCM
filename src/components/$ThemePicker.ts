import { $Node, $node, $svg, $text, attr, Behavior, component, event, INodeElement, O, style, stylePseudo } from "@aelea/core"
...
export const $AppSettings = (themes: Theme[]) => component((
  [sampleChangeThemeEffect, changeThemeEffect]: Behavior<any, any>,
  [sampleDisposeAccount, disposeAccount]: Behavior<any, any>,
) => {
  ...
  const disposeAccountBehavior = O(
    applyOps,
    sampleDisposeAccount(
      event('click')
    ),
    sampleChangeThemeEffect(
      event('click'),
  ...
  const $control = ({ label, $content }: { label:string, $content: $Node }) => $row(layoutSheet.spacingSmall, style({ alignItems: 'center' }))(
    $content,
    $text(label)
  )
  ...
  return [
    merge(
      $row(layoutSheet.spacingBig, style({ position: 'absolute', left: '15px', top: '15px', cursor: 'pointer' }))(
        disposeAccountBehavior(
          $control({
            $content: $trash,
            label: 'Dispose Account'
          })
        ),
        changeThemeBehavior(
          $control({
      ...
      merge(changeThemeEffect, disposeAccount)
    )
